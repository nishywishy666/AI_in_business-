...
- **Periods:** Overview and Analytics each carry a Today / This week / This month selector (plan 0014); Analytics also has Custom. `week` is a rolling 7 days, `month` is calendar month-to-date, and the untouched `30d` key still means a rolling 30 days for API callers.
