...
5. If you cannot find the parent questionnaire path, implement `CONTEXT_PATH` as an env override. Default: `users/{userId}/context`.  
...
1. **Not a standalone app.** Ship as an importable package `marketing_radar` started by the parent process. No `python app.py` server of our own. No first-visit questionnaire UI.  
2. **Shared Firebase only.** Same project as the overlord. Writes exclusively under `users/{userId}/marketingRadar/**`. Read-only on `users/{userId}/context` (or `CONTEXT_PATH`).  
3. **No Supabase. No SQLite as source of truth.** Local disk is a cache, not the database.  
4. **All AI \= Gemini free tier.** One `GEMINI_API_KEY`. No `ANTHROPIC_API_KEY`. No Gemini Pro. Do not enable paid Gemini billing from this module.  
5. **ScrapeCreators budget:** 100 free credits, never expire, 1 credit ≈ 1 HTTP request, cached identical request within 48h \= 0 credits. Max **3 live** paid calls per scan. **15-credit reserve** for Like-transcripts. Never paginate. Never comments. Never audience-demographics. Never TikTok `use_ai_as_fallback` (10 credits).  
6. **Do not call** `GET /v1/account/credit-balance` on dashboard refresh. It may cost 1 credit. Use `credits_remaining` on the last paid response. Call balance only if we have no cached figure **and** a scan is about to run.  
7. **Firebase reads for agent/UI:** at most **one pull per calendar day** of context \+ latest ScanBrief \+ usage snapshot. Split into local cache. Re-fetch only if cache missing, older than 24h, or explicitly invalidated after a write we just made (scan, like, save).  
8. **Cross-platform:** `pathlib` only. No hardcoded `/Users/...` or `C:\`. APScheduler inside the parent process. No launchd, no Windows Task Scheduler, no bash `install.sh`.  
9. **Auth:** use the parent’s `userId`. Do not build a second auth system.  
10. **Out of scope forever for v1:** video generation, publishing, Telegram, Claude Code, Remotion, Higgsfield, Facebook “global trending”, writing back into the main context doc.
...
- One **ScanBrief document per scan** (the packet). After daily pull, Python splits it into local cache keys: `weekly_take`, `global`, `niche`, `film_this`, `playbook`, `profile`, `credits`.  
...
`Path(platformdirs.user_cache_dir("marketing_radar")) / user_id /`

Files:

- `context.json` — ContextProfile  
- `brief.json` — full ScanBrief  
- `brief_parts/` — one JSON file per split key  
...
### 9.1 ScanBrief — stored at `scans/{scanId}` and returned by `get_brief()`
...
`scan_id` format: `YYYY-MM-DD` of the scan’s UTC date. If two scans somehow share a date, suffix `-2`. **Never overwrite** an existing `scans/{scanId}`; a new scan is a new document. Update `latest` to point at it.
...
`post_id` \= `{platform_short}_{provider_id}` so TikTok and Instagram cannot collide.
...
On save: `status=saved`, `saved_at=now`, `expires_at=null`.  
Daily job deletes `status==draft AND expires_at < now`. Saved rows are never auto-deleted.
...
| Platform | Path | Notes |
| :---- | :---- | :---- |
| TikTok | `GET /v1/tiktok/video/transcript` | Never `use_ai_as_fallback` |
...
- `request_hash = sha256(method + url + sorted(params))`  
- If `scrapeCache/{hash}` exists and `expires_at > now`: return cached JSON, `credits_charged = 0`, do not increment spend.  
- Else: HTTP GET, store raw JSON \+ `expires_at = now+48h`, read `credits_remaining` / `credits_charged` from the body, append `usage/events`.  
...
- Write a new ScanBrief with `kind=offday_recap` (new scan\_id, e.g. `{date}-recap`). Still a new document. Update `latest`.
...
- Do **not** use `higgsfield_producibility`.  
- Always apply **relative** velocity (the `--relative` path).  
- Multiply by `niche_fit` and `platform_weight`.
...
Facebook `view_count` may be null. Do not use views in `weighted`. If likes missing, fall back to reactions \+ comments \+ shares.
...
Dedup key `(platform, provider_id)` — keep higher `final`.  
...
- **niche:** top 3–5 with `niche_fit >= 0.5` (if fewer than 3, take top by `final` anyway and let Gemini say they are weak)  
- **film\_this candidate:** highest `final` among TT/IG unless a YT/FB item wins by ≥ 0.15  
- Dedup against `post_id`s seen on scans in the last 14 days (skip if already shown, unless it is the only film\_this candidate)

Gemini may **swap** `film_this` to another of the top \~15 ids. It may not invent a new `post_id`.
...
| :---- | :---- | :---- | :---- |
| 1 | gemini-3.8-flash, gemini-3-flash, gemini-3.5-flash | high | 20 (or 1500 if `GEMINI_LADDER_JSON` says so) |
...
Caps are **our counters**, Pacific day (`America/Los_Angeles`). Official reset: midnight Pacific. We do not scrape AI Studio.

Algorithm for every call:

1. Load `usage/geminiDaily/{pacificDate}`.  
2. Starting at the first rung with `used < cap` and not marked exhausted: POST.  
3. On **429** or our remaining \== 0: mark exhausted, try **next** rung **once**. No retry loop on the same model.  
4. On success: increment `used`, write `usage/events` with `purpose`, `model`, `tokens` if present.  
5. If chosen quality is medium/low: attach `quality_warning`.  
6. If ladder empty: raise `GeminiExhausted` with `resets_at`. Callers must still return ranked posts / last brief. Never invent prose.
...
- Every `post_id` must be in the provided list.  
- Do not invent view counts or claim a platform that is not on the packet.  
- Prefer TT/IG for `film_this` unless a secondary platform is clearly stronger.  
- Write for the user’s niche and goal.

Merge `cards[]` onto `posts/{id}`. Append `playbook_delta` as new playbook docs. Persist ScanBrief. Update `latest`. Recompute usage snapshot \+ alerts. Write-through local cache.

### 12.3 Like → angles → script

Triggered from Marketing UI or chat tool `like_trend`.

1. Set `posts/{id}.liked = true`.  
2. Transcript: YouTube → free library. Else if remaining \> 15 and duration ≤ 120s → 1 allowlisted transcript call. Else caption \+ metrics only.  
3. One Gemini call, purpose=`angles`: 3 original angles that **keep the format** and **change the topic** for this niche. User (or chat) picks one.  
4. One Gemini call, purpose=`script`: `filming_guide` \+ spoken `script` \+ caption for the chosen angle.  
5. Insert `scripts/{uuid}` as `draft`, `expires_at = now + 14d`.  
6. Point `posts/{id}.script_id` at the draft.
...
Overlord must **not** use this chat loop. Overlord calls `services.brief.get_brief()` / `services.stats.get_stats()` only.
...
`GET /api/stats` (or `get_stats()`) returns:
...
- remaining \== 0 → exhausted; skip free YT pull until `resets_at`

**Gemini:**

- active remaining ≤ 20% of that cap → warning (include next model \+ `resets_at`)  
...
`stale=true` if snapshot `checked_at` is older than 26 hours (daily pull missed).
...
1. Empty state if no ScanBrief yet.  
2. Board: `weekly_take`, Global cards, Niche cards, Film this.  
3. Card actions: Like → pick 1 of 3 angles → show guide/script → Save.  
4. Saved script library (`status=saved`).  
5. Side chat bound to marketing agent.  
6. Header pills from `get_stats()`: SC remaining, each Gemini rung used/remaining, active model, quality warning, `resets_at`, YouTube remaining.  
7. Banners from `alerts[]`. Copy must say whether credits **reset** (Gemini/YouTube) or **do not reset** (ScrapeCreators).

Overlord responses about marketing must cite the same `scan_id` and must not scrape.
...
1. `config.py` \+ `db/paths.py` \+ `db/firebase.py` (read/write under prefix only; unit-test that a write to `users/{uid}/settings` is impossible through the client).  
2. `packets/schema.py` \+ `cache/local.py` \+ `jobs/daily_pull.py`.  
3. `context/parse.py` with fixtures: Firestore map and a sample `context.md`.  
4. `scrapers/endpoints.py` \+ `scrapers/client.py` with a mock HTTP layer; prove allowlist \+ 48h cache \+ no credit-balance on get\_stats.  
5. Free adapters (YouTube / Reddit / Trends) behind the same normalize interface.  
6. `scoring/*` — port relative velocity; golden-test against a fixture copied from AIOS ideas (drop producibility; inject niche\_fit=1, platform\_weight=1 and assert midrank matches).  
7. `jobs/scan.py` rotation table \+ credit shedding.  
8. `agent/gemini.py` ladder with a fake transport (429 on rung 1 → rung 2).  
9. Synthesis merge → ScanBrief write → `latest` → cache write-through.  
10. `usage/snapshot.py` \+ `alerts.py`.  
11. Like / script / expire\_drafts.  
12. Chat tools.  
13. Parent route wrappers \+ Marketing UI fragment.  
14. Overlord read helpers (`get_brief`, `get_stats`) documented in a 20-line `OVERLORD.md`.

---

## 17\. Acceptance tests (must all pass)

1. Writing through `firebase.py` to any path not starting with `users/{uid}/marketingRadar` raises.  
2. Daily pull is a no-op if cache `pulled_date` is today and files exist; deleting one file forces re-fetch.  
3. A paid scan issues ≤ 3 allowlisted ScrapeCreators URLs. A second identical URL inside 48h hits cache (0 credit).  
4. `credit-balance` is never called from `get_stats()`.  
5. Relative scoring: three posts with raw 10, 20, 20 produce midrank velocities matching `score-ideas.py` (`--relative`).  
6. `final` does not include a producibility term.  
7. Call 3 alternates FB vs YT across four successive `scan_index` values per the table. Missing FB URLs never call Facebook endpoints.  
8. remaining=15 on scan start → 2 calls (no Call 3). remaining=0 → 0 paid calls.  
9. Gemini 429 on best model → exactly one fallback call; snapshot shows quality warning \+ `resets_at` midnight PT.  
10. Synthesis JSON with an unknown `post_id` is rejected; ScanBrief is not written.  
11. Like with remaining=15 does not call transcript endpoints.  
12. Draft older than 14 days is deleted; `status=saved` with old `created_at` is kept.  
13. Overlord helper does not import scrapers or gemini.generate.  
14. Package starts on Windows and macOS with the same `start_marketing_radar()` call.
...
| Condition | Behaviour |
| :---- | :---- |
| No context doc | Empty state; do not scrape; alert `context_missing` |
...
| Malformed Gemini JSON | One repair prompt on a lower rung only; then persist scored cards without `weekly_take` and set `kind` note |
